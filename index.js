...
const DISURL_ID = '813077581749288990';
...
const DISURL_INTERVVAL = 30 * 60 * 1000 //30 mins
...
async function checkAndBump(pair) {
    ...
    if (useDiscadia) {
        ...
        const lastDisurlBump = await getLastBumpTime(index, 'Disurl');
        if (lastDisurlBump === 0) {
            // No previous bump record, bump immediately
            console.log(`[Bot ${index}] No previous Disurl bump found, bumping now...`);
            await performBump(token, channelId, index, DISURL_ID, 'Disurl');
        } else {
            const timeUntilNextBump = DISURL_INTERVVAL - (now - lastDisurlBump);
            if (timeUntilNextBump <= 0) {
                console.log(`[Bot ${index}] Time for Disurl bump (${Math.abs(Math.floor(timeUntilNextBump/60000))} minutes overdue)`);
                await performBump(token, channelId, index, DISURL_ID, 'Disurl');
            } else {
                console.log(`[Bot ${index}] Next Disurl bump in ${Math.floor(timeUntilNextBump/60000)} minutes`);
            }
        }
